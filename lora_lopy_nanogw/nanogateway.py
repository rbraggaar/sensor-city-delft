...
class NanoGateway:
    ...
    def _make_node_packet(self, rx_data, rx_time, tmst, sf, rssi, snr):
        RX_PK["rxpk"][0]["time"] = "%d-%02d-%02dT%02d:%02d:%02d.%dZ" % (
            rx_time[0], rx_time[1], rx_time[2], rx_time[3], rx_time[4], rx_time[5], rx_time[6])
    ...
    def _push_data(self, data):
        token = os.urandom(2)
        packet = bytes([PROTOCOL_VERSION]) + token + bytes([PUSH_DATA]) + \
            binascii.unhexlify(self.id) + data
        with self.udp_lock:
            try:
    ...
    def _pull_data(self):
        ...
        packet = bytes([PROTOCOL_VERSION]) + token + \
            bytes([PULL_DATA]) + binascii.unhexlify(self.id)
        with self.udp_lock:
            try:
                self.sock.sendto(packet, self.server_ip)
            except Exception:
    ...
    def _ack_pull_rsp(self, token, error):
        ...
        packet = bytes([PROTOCOL_VERSION]) + token + bytes([PULL_ACK]) + \
            binascii.unhexlify(self.id) + resp
        with self.udp_lock:
            try:
                self.sock.sendto(packet, self.server_ip)
            except Exception:
    ...
    def _lora_cb(self, lora):
        ...
        if events & LoRa.RX_PACKET_EVENT:
            ...
            stats = lora.stats()
            self._push_data(self._make_node_packet(rx_data, self.rtc.now(),
                                                   stats.timestamp, stats.sf, stats.rssi, stats.snr))
            self.rxfw += 1
